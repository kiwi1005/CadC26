...
def beam_rollout(
    ...
    *,
    ...
    start_state = _clone_state(initial_state) if initial_state is not None else ExecutionState()
    beam = [_BeamState(score=0.0, state=start_state, actions=[])]
    ...
    for _step in range(max_steps):
        ...
        for item in beam:
            ...
            output = policy(
                ...
                placements=item.state.placements,
            )
            ...
            scored = sorted(
                ((score_action_with_policy(output, c), c) for c in candidates),
                key=lambda x: x[0],
                reverse=True,
            ...
            for score, candidate in scored:
                ...
                expanded.append(
                    _BeamState(
                        score=item.score + float(score),
                        state=new_state,
                        actions=item.actions + [candidate],
                    )
                )
    ...
    if all_blocks_placed:
        summary = summarize_hard_legality(
            case, [best.state.placements[idx] for idx in range(case.block_count)]
        )
