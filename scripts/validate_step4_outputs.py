...
def _validate_step6g_input(path: Path) -> dict[str, Any]:
    payload = _read_json(path)
    if payload is None:
        return {"overall_status": "fail", "reason": f"missing input {path}"}
    required = {
        "case_ids",
        "seeds",
    ...
    checks = {
        ...
        "exact_case_ids": payload.get("case_ids") == [0, 1, 2, 3, 4],
        "exact_seeds": payload.get("seeds") == [0, 1, 2],
        "semantic_complete": float(payload.get("semantic_completion_fraction", 0.0)) == 1.0,
        "fallback_below_gate": float(payload.get("fallback_selected_fraction", 1.0)) < 0.30,
        "hard_feasible": int(payload.get("hard_infeasible_after_repair_count", 1)) == 0,
        "has_utility_gain": (
            float(payload.get("hpwl_proxy_relative_improvement", 0.0)) >= 0.05
            or float(payload.get("bbox_area_proxy_relative_improvement", 0.0)) >= 0.05
            or float(payload.get("soft_violation_relative_reduction", 0.0)) >= 0.20
        ),
        "has_constructive_pre_repair": bool(payload.get("constructive_pre_repair_metrics")),
        "has_post_repair": bool(payload.get("post_repair_metrics")),
        "has_repair_displacement": "repair_displacement_mean" in payload,
    }
    return {
        "input": str(path),
        "missing": missing,
...
def main() -> None:
    ...
    if known.input:
        result = _validate_step6g_input(Path(known.input))
    ...
    payload = {
        ...
        "artifacts_checked": {
            "followup": str(
                (RESEARCH / "generalization_followup_smallcheckpoints.md").relative_to(SHARED_ROOT)
            ),
            "delta": str(
        ...
        "observed": {
            ...
            "best_of_k_rules_present": sorted(best_of_k_json.get("aggregate_by_rule", {}).keys())
